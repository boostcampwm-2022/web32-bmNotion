...
const sampleBlocks: BlockInfo[] = [
    { blockId: 1, index: 1, content: "", type: "TEXT" },
]
...
export default function PageComponent(): React.ReactElement {
    ...
    const addBlock = ({ type, content, index }: { type: string, content: string, index: number }) => {
        ...
        setPageInfo(prev => ({
            ...prev,
            blocks: [...prev.blocks, { content, type, index, blockId: prev.nextId }].map(block =>
                ({ ...block, index: updateIndex(block.index) })).sort(sortedByIndex),
    ...
    return (
        <PageBox>
            {pageInfo.blocks.map((block, idx) =>
                <BlockContent key={block.blockId} blockId={block.blockId} newBlock={addBlock} changeBlock={changeBlock} index={block.index} type={block.type} />)
            }
        </PageBox>
    )
}
